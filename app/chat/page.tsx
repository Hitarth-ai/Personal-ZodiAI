...
function Chat() {
  ...
  // Auto-start chat if query params exist (from landing page demo)
  useEffect(() => {
    if (hasAutoStartedRef.current) return;

    // Only proceed if we have at least a name and day (basic validation)
    const pName = searchParams.get("name");
    const pDay = searchParams.get("day");

    if (pName && pDay && status === "ready" && messages.length === 0) {
      hasAutoStartedRef.current = true;

      const newDetails: BirthDetails = {
        name: pName || "",
      ...
      // Construct start message immediately
      const summary = `My name is ${newDetails.name}. My date of birth is ${newDetails.day}-${newDetails.month}-${newDetails.year} at ${newDetails.hour}:${newDetails.minute}. I was born in ${newDetails.place}. Please use Vedic astrology to interpret my chart, then show me a menu of what ZodiAI can help me with.`;
  ...
  }, [searchParams, sendMessage, activeId, language, status, messages.length, router]);
